...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_acc, model):
        print(f'Validation accuracy increased ({self.best_score:.6f} --> {val_acc:.6f}). Saving model...')
...
class ImageClassifier:
    def __init__(self, data_dir, image_size=224, batch_size=32, num_workers=4, 
                ...
                early_stop_patience=5, device=None, use_augmentation=False):
        ...
        self.use_augmentation = use_augmentation  # New parameter to control augmentation
    ...
    def setup_data_transforms(self):
        ...
        # Validation transform never uses augmentation
    ...
    def load_datasets(self):
        ...
        with open(os.path.join("results", "classes.txt"), 'w') as f:
    ...
    def evaluate_hard_test(self, hard_test_dir):
        """Evaluate model on hard test cases"""
        print(f"\nRunning Hard Test Evaluation on: {hard_test_dir}")
        
        try:
            # Verify the directory structure
            subdirs = ['Bird', 'Drone', 'Plane', 'UAV']
            for subdir in subdirs:
                if not os.path.exists(os.path.join(hard_test_dir, subdir)):
                    raise FileNotFoundError(f"Missing required subdirectory: {subdir}")

            # Setup transforms for hard test (same as validation - NO AUGMENTATION)
            ...
            class_indices = hard_dataset.class_to_idx
            class_names = hard_dataset.classes
            
            # Evaluate
            self.model.eval()
            all_preds = []
            all_labels = []
            all_probs = []  # Store probabilities for more detailed analysis
            ...
            present_class_indices = list(set(all_labels))
            present_class_names = [self.classes[i] for i in present_class_indices]

            report = classification_report(
                all_labels, 
                all_preds, 
                target_names=present_class_names,  # Only use present classes
                labels=present_class_indices,      # Specify which labels to include  
                output_dict=True,
                zero_division=0
            )
            ...
            with open(report_file, 'w', encoding='utf-8') as f:
                ...
                for class_idx in present_class_indices:
                    ...
                    class_probs = np.array(all_probs)[class_mask].tolist()
                    ...
                    # Individual image results with probabilities
                    f.write("IMAGE RESULTS:\n")
                    f.write("-"*20 + "\n")
                    for filename, pred, probs in zip(class_files, class_preds, class_probs):
                        ...
                        for i, cls in enumerate(class_names):
                            if i < len(probs):
                                f.write(f"      {cls}: {probs[i]*100:.2f}%\n")
            
            print(f"Hard test report saved to: {report_file}")
            
            # Create confusion matrix for hard test
            cm = confusion_matrix(all_labels, all_preds, labels=present_class_indices)
            plt.figure(figsize=(10, 8))
            sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            ...
            plt.yticks(rotation=45)
            plt.tight_layout()
            self.save_plot(plt.gcf(), "hard_test_confusion_matrix.png")
            
        except Exception as e:
            print(f"\nError during hard test evaluation: {str(e)}")
            print("Please verify:")
            print(f"1. The hard_test directory exists at: {hard_test_dir}")
            print("2. It contains subdirectories: bird, drone, plane, UAV")
    ...
    def train(self):
        ...
        # Run hard test evaluation (only if folder exists)
        hard_test_dir = r"C:\Users\omrib\Desktop\omri\hard_test"
        if os.path.exists(hard_test_dir):
            self.evaluate_hard_test(hard_test_dir)
        else:
            print(f"\nHard test directory not found at {hard_test_dir}, skipping hard test evaluation")

def main():
    config = {
        'data_dir': r"C:\Users\omrib\Desktop\Drone Project\project-no-results\img_set",  # Change this to your data directory
        'image_size': 224,
        'batch_size': 64,
        'num_workers': 8,
        'lr': 3e-4,
        ...
        'use_augmentation': False  # Disable augmentation for pre-augmented data
    }
